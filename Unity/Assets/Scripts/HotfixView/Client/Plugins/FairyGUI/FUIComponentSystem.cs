...
namespace ET.Client
{
    ...
    [FriendOf(typeof (FUIEventSingleTon))]
    ...
    public static partial class FUIComponentSystem
    {
        ...
        {
            if (!self.VisiblePanelsDic.TryGetValue((int)id, out EntityRef<FUIEntity> fuiEntity))
            {
                return;
            }

            if (((FUIEntity)fuiEntity).IsUsingStack)
            {
        ...
        private static void UnLoadPanel(this FUIComponent self, PanelId id, bool isDispose = true)
        {
            ...
            IFUIEventHandler handler = FUIEventSingleTon.Instance.GetUIEventHandler(id);
        ...
        {
            if (self.AllPanelsDic.TryGetValue((int)id, out EntityRef<FUIEntity> fuiEntity))
            {
                return fuiEntity;
            }
        ...
        {
            // if (self.Root().GetComponent<FUIEventSingleTon>().PanelTypeInfoDict.TryGetValue(typeof (T).Name, out PanelInfo panelInfo))
            if (FUIEventSingleTon.Instance.PanelTypeInfoDict.TryGetValue(typeof (T).Name, out PanelInfo panelInfo))
            {
        ...
        public static void CloseAllPanel(this FUIComponent self)
        {
            ...
            foreach (KeyValuePair<int, EntityRef<FUIEntity>> panel in self.AllPanelsDic)
            {
        ...
        {
            self.FUIEntitylistCached.Clear();
            foreach (KeyValuePair<int, EntityRef<FUIEntity>> panel in self.VisiblePanelsDic)
            {
                if (((FUIEntity)panel.Value).PanelCoreData.panelType == UIPanelType.Fixed && !includeFixed)
                {
                    continue;
                }

                if (((FUIEntity)panel.Value).IsDisposed)
                {
                    continue;
                }

                self.FUIEntitylistCached.Add((PanelId)panel.Key);
                ((FUIEntity)panel.Value).GComponent.visible = false;
                FUIEventSingleTon.Instance.GetUIEventHandler(((FUIEntity)panel.Value).PanelId).OnHide(panel.Value);
            }
        ...
        private static void RealShowPanel(this FUIComponent self, FUIEntity fuiEntity, PanelId id, Entity contextData = null)
        {
            ...
            FUIEventSingleTon.Instance.GetUIEventHandler(id).OnShow(fuiEntity, contextData);
        ...
        private static bool CheckDirectlyHide(this FUIComponent self, PanelId id)
        {
            ...
            if ((FUIEntity)fuiEntity != null && !((FUIEntity)fuiEntity).IsDisposed)
            {
                ((FUIEntity)fuiEntity).GComponent.visible = false;
                FUIEventSingleTon.Instance.GetUIEventHandler(id).OnHide(fuiEntity);
        ...
        {
            if (!FUIEventSingleTon.Instance.PanelIdInfoDict.TryGetValue(fuiEntity.PanelId, out PanelInfo panelInfo))
            {
                Log.Error($"{fuiEntity.PanelId} panelInfo is not Exist!");
                return;
            }

            IFUIEventHandler fuiEventHandler = FUIEventSingleTon.Instance.GetUIEventHandler(fuiEntity.PanelId);
            ...
            {
                var (_, translateFUI) = self.Root().GetComponent<LocalizeComponent>().GetCurrentTranslator();
        ...
        {
            foreach (KeyValuePair<int, EntityRef<FUIEntity>> kv in self.AllPanelsDic)
            {
        ...
        Func<string, string, string> translator)
        {
            if (fuiEntity == null || fuiEntity.IsDisposed)
            {
                return;
