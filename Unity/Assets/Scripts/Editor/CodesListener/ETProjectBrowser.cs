...
{
    [MenuItem("ET/ET Project Menus/ShowScriptsNodes")]
    ...
    public string ListenFolderPath = "../Unity/Assets/Scripts";
    ...
    {
        _watcher = new FileSystemWatcher(ListenFolderPath);
            ...
            else
            {
                if (render)
                {
                    ...
                    if (GUILayout.Button(node.Name, new GUIStyle("Label")))
                    {
                        ...
                        if ((DateTime.Now - lastClickTime).TotalSeconds < 0.5 && !string.IsNullOrEmpty(_clickToken.name) &&
                            _clickToken.name == node.FullName)
                        {
    ...
    {
        TreeNode root = new TreeNode(this.ListenFolderPath);
