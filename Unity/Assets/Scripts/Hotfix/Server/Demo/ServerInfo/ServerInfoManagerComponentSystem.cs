namespace ET.Server
{
    ...
    public static partial class ServerInfoManagerComponentSystem
    {
        ...
        public static void Load(this ServerInfoManagerComponent self)
        {
            ...
            var startZoneConfigs = StartZoneConfigCategory.Instance.DataList;
            foreach (StartZoneConfig startZoneConfig in startZoneConfigs)
            {
                if (startZoneConfig.Type != ZoneType.Game)
                    continue;
                ServerInfo newServerInfo = self.AddChildWithId<ServerInfo>(startZoneConfig.Id);
                newServerInfo.ServerName = startZoneConfig.ZoneName;
                newServerInfo.Status = startZoneConfig.Status;
